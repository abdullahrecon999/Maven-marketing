...
const ChatSideBar = ({setCurrent}) => {
    ...
    useEffect(()=>{
      ...
      const fetch = async()=>{
        setIsloading(true)
        const response = await axios.get(`http://localhost:3000/chats/getContacts/${user?._id}`,
          {headers: {
  ...
  return (
    <React.Fragment>
     <div className='flex flex-col w-[25%] border-r-2 h-[10%] '>
        <h1 className=' text-base mx-3 mt-3 mb-4 text-gray-700'> Contacts</h1>
        {data?.data?.data[0]?.contacts?.lenght===0? <div className='flex flex-col px-3 py-12'>
          <h1> your contacts appear here</h1>
            ...
            {data?.data?.data[0]?.contacts.map((item, i)=>{
                ...
                }} key={i} className="flex flex-col items-start p-3 px-5 py-5 border w-[100%] hover:bg-grey">
                    <h1  className='text-base '>{item.contact.name}</h1>
                </div>)
            })}
        </div>}
