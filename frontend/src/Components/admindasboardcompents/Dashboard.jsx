...
const Dashboard = () => {
  ...
  const {
    ...
    refetch,
    isError,
  } = useQuery("dashboard", fetchDashboard, { onCompleted: setDashData });
      ...
      <div className="flex space-x-12 justify-center items-center py-5 px-14">
        <>
          ...
          <Box
            ...
            data={dashboard?.data.val?.activationRequests}
          ></Box>
          <Box
            title="Influencers"
            data={dashboard?.data.val?.influencers}
          ></Box>
          <Box title="Brands" data={dashboard?.data.val?.brands}></Box>
        </>
      </div>
      {isError && (
        <h1 className="text-red-600">ooooopss!! something went wrong</h1>
      )}
