...
const BidModal = ({ data, brand, id, influencer, onClose }) => {
  ...
  const handleSubmit = async (values) => {
    ...
    return await axios.post(
      `http://localhost:3000/influencer/bidCampaign/${id}`,
      val,
      {
        headers: {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center backdrop-blur-sm bg-30 z-50">
      <Formik
        initialValues={{
          discription: "",
          amount: "",
          ...
          <Form>
            ...
            <div className="flex bg-white flex-col items-center justify-center border py-8 h-[80vh]">
              <button
                type="button"
                onClick={() => handleClose()}
                className="place-self-end px-5 pt-3"
              >
                {" "}
                <CloseIcon></CloseIcon>
              </button>
              <div className="w-[70%] h-full  bg-white p-4 border rounded-xl overflow-y-auto ">
                <div className="space-y-2">
                  {isSuccess ? (
                    <p className="text-red">Proposal submitted successfully</p>
                  ) : null}
                  {isError && (
                    <p className="text-red-800">{console.log(status)}</p>
                  )}
                  <h1 className="text-2xl text-black font-railway mb-1">
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      className="block p-2 text-gray-900 bg-gray-50 border rounded-lg border-gray-300 "
                      required
                      onChange={(e) => {
                  ...
                  <div>
                    <h1 className="text-xl text-black font-railway">
                      Questions <span className="text-xl text-red-500">*</span>
                    </h1>
                    ...
                    <div className="flex flex-col px-3 py-4 border rounded-md shadow-md my-4">
                      {data.map((item, i) => {
                        return (
                          <div>
                            <h1 className="text-base text-black font-railway mb-3">
                              {i + 1} {item}{" "}
                              <span className="text-xl text-red-500">*</span>
                            </h1>
                            <textarea
                              className="block mb-4 p-2 w-full text-gray-900 bg-gray-50 border rounded-lg border-gray-300 "
                              required
                              onChange={(e) => {
