...
const InfluencerAllCampaigns = () => {
  ...
  const fetchCampaigns = async () => {
    const res = await fetch('http://localhost:3000/campaign/allcampaigns?'+decodeURIComponent(searchParams));
    return res.json();
  };

  const { data:campaign, isLoading, isSuccess, refetch, isError } = useQuery(['campaign',search], fetchCampaigns);
